 ...
 * @param {Object} ctx 页面栈或组件的实例，默认为当前页面栈实例
 */
const getCtx2 = (parent,selector, ctx = getCurrentPages()[getCurrentPages().length - 1]) => {
  let componentCtx = null;
  if(parent){
    componentCtx = ctx.selectComponent(parent).selectComponent(selector);
  }else{
    componentCtx = ctx.selectComponent(selector);
  }
  if (!componentCtx) {
    throw new Error('无法找到对应的组件，请按文档说明使用组件')
  }
...
const $stopWuxRefresher = (parent,selector = '#wux-refresher', ctx) => getCtx2(parent,selector, ctx).finishPullToRefresh()
const $stopWuxLoader = (parent,selector = '#wux-refresher', ctx, isEnd) => getCtx2(parent,selector, ctx).finishLoadmore(isEnd)
